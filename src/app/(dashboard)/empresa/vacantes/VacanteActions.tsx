...
export default function VacanteActions({ jobId, status }: VacanteActionsProps) {
  ...
  return (
    <div className="flex flex-wrap gap-2 shrink-0">
      ...
      {status === 'activa' ? (
        <Button size="sm" variant="secondary" onClick={() => updateStatus('pausada')} loading={loading === 'pausada'}>
          <Pause className="w-3.5 h-3.5" />
          Pausar
        </Button>
      ) : (
        <Button size="sm" variant="secondary" onClick={() => updateStatus('activa')} loading={loading === 'activa'}>
